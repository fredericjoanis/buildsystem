...
--
	...
	function os.isARM()
		if os.is("linux") then
			...
			arch = arch:lower()
			for _, hosttype in ipairs(_ARMHostTypes) do
				if arch:find(hosttype) then
					return true
				end
			end
...
function os.copy (from, to)
	-- make sure to is always recognized as a folder
	to = to .. "/"
	local fromAbs = path.getabsolute(from)
	os.mkdir( path.getdirectory(to) )

    if os.isdir(fromAbs) then
		from = from .. "/**"
	end

	local dirs = os.matchdirs(from)
    for _, dname in ipairs(dirs) do
        os.copy(dname, to .. "/" .. path.getname(dname) .. "/" )
    end
        
    local files = os.matchfiles(from)
    for _, fname in ipairs(files) do
        if path.getdirectory(from) == path.getdirectory(fname) then
            os.copyfile(path.getabsolute(fname), to .. "/" .. path.getname(fname) )
        end
    end
end